...
**Fathom row 3.1.0.15 (follow-on) — `overrides` parent-type nested-type qualification, the last RC3 instance.** A clean rebuild of a real C# app under the armed strict edge gate went from 34 unresolvable edges down to exactly 1: an implementing method in a NESTED type (`Smtp.SmtpWrapper`) overriding an interface member ALSO nested under the same outer scope (`Smtp.ISmtpClient`). The target element's own key is qualified by `GetQualifiedRawName` walking the full `outer/inner` nesting (`smtp/ismtpclient/send-mailmessage`), but the `overrides` emit block built its parent-type qualifier from the parent type's SHORT `.Name` (`ismtpclient`) — the heritage/`references`/generic-constraint paths were routed through `GetQualifiedRawName` in 0.63.0, but the `overrides` parent-type name was never migrated. Same-file edges emit no `targetRef` and rely on ingest's self-file key synthesis from `targetName`, so the unqualified emit was a permanent dangler; the cross-file `targetRef` path shared the same defect.

### Fixed

- **`src/Program.cs` (~line 2543, `overrides` emission block).** For IN-SOURCE parents (`parentDeclRef != null` — the overridden member has a declaring syntax reference), `parentTypeName` is now derived by resolving the parent's `ContainingType` to its declaring syntax node and running it back through `GetQualifiedRawName` — the same machinery the heritage/references/generic-constraint paths already use — instead of the symbol's short `.Name`. EXTERNAL parents (no declaring syntax reference at all — BCL/NuGet) are left untouched: that branch is informational-only (`metadata.external`, no `targetRef`), so short-name qualification was never wrong there.

### Tests

- **`tests/OverridesEdgeTests.cs`** — two new fixtures: `NestedInterfaceImplementation_SameFile_TargetNameCarriesOuterQualifier` (the SMTP.cs shape — same-file, no `targetRef`, asserts the emitted `targetName` carries the outer qualifier so ingest's self-file synthesis lands on the target element's real key) and `NestedBaseType_CrossFile_TargetRefMatchesTargetElementNaturalKey` (the cross-file half — a nested base type in a different file, asserts the emitted `targetRef` byte-matches the target element's `naturalKey`). TDD RED witnessed first against the pre-fix build: `Assert.Equal` failures, `"smtp/ismtpclient/send-mailmessage"` expected vs `"ismtpclient/send-mailmessage"` actual (same-file), and `"...container:widgetbase:render"` expected vs `"...widgetbase:render"` actual (cross-file). GREEN after the fix. The pre-existing `HeritageExternalTaggingTests.ExternalOverride_OverridesEdge_ExternalTagged` fixture (external `object.Equals`/`GetHashCode` override) stays green unmodified, confirming the fix does not disturb the EXTERNAL-parent path.
- Class-first sweep: grepped every `.ContainingType.Name` / short-`.Name`-for-a-type-symbol site in `src/*.cs` — the `overrides` parent-type qualifier fixed here was the only one still building a key from a short name where an in-source target requires `GetQualifiedRawName`'s qualified form; every other site (`ResolveCallTarget`, `EmitTypeRefEdge`'s heritage/references/generic-constraint verdicts, `ResolveExternalCallName`/`ResolveExternalPropertyName`) either already routes through `GetQualifiedRawName` for in-source targets or is gated to the EXTERNAL-only branch where the short name is correct by contract.
- Full suite: **300/300 pass** (was 298, +2 from this row's fixtures). `dotnet build src -c Debug` clean; `npm run build` (`dotnet publish`) clean.
...
- **Conservation-safe Limitation kind (finding #1, HIGH).** `RecordUnresolvedTypeRef` minted `kind: "unresolved-type-ref"` — NOT a member of the registered Limitation taxonomy (`nodegraph-limitations/src/schema.ts`, `overlay.ts` `ALL_KINDS`, `nodegraph-analysis/src/conformance-types.ts` `LimitationKind`). `insertLimitation` → `validateInput` throws on an unregistered kind; `ingestFromArtifacts` catches and drops the throw — so EVERY drop-path Limitation from the 0.63.0 fix vanished silently at overlay ingest instead of hard-failing or surfacing, a conservation break (`IN ≠ ADMITTED + REJECTED(named)`). Now emits the pre-existing, already-registered `kind: "unresolved-reference"` ("Type/field reference unresolvable.") — verified present in all 3 taxonomy layers before relying on it, per instruction.
- **Type parameters are not-applicable, never unresolved (finding #2, MEDIUM — no-silent-degradation §5).** `T Get<T>(T item)` and `class F<T,U> where T:U {}` resolve `T`/`U` to `ITypeParameterSymbol` — a fully-resolved symbol, not a resolution failure — but `ResolveTypeRef` only recognized `INamedTypeSymbol`, so a type parameter fell into the SAME `null` verdict as a genuine unresolvable reference, wrongly minting an `unresolved-reference` Limitation for every generic return type, parameter type, and constraint bound to another type parameter. `ResolveTypeRef`'s return type is now a closed 4-member `TypeRefVerdictKind` enum (`InSource` / `External` / `NotApplicable` / `Unresolved`) instead of a nullable tuple — a `null` that silently meant two different things is exactly the bug this refactor removes. `NotApplicable` emits neither an edge nor a Limitation; callers (`EmitTypeRefEdge` and the heritage call site) switch exhaustively on the enum (`default` throws — no silently-ignored case).
- **External generic targets no longer carry a stray `global` token (finding #3).** `List<MyUserType>` canonicalized to `system-collections-generic-list-global-myusertype` because only a LEADING `global::` was stripped from the FQN — Roslyn's `FullyQualifiedFormat` also prefixes a generic type ARGUMENT with `global::` (`List<global::MyUserType>`), which a `StartsWith`/`Substring` strip never reaches. The external branch of `ResolveTypeRef` now builds the FQN with `SymbolDisplayFormat.FullyQualifiedFormat.WithGlobalNamespaceStyle(SymbolDisplayGlobalNamespaceStyle.Omitted)`, which omits `global::` everywhere in the display string, not just at the front.
- **Decision confirmed, not a code change (finding #3a).** External generic `references` edges (`List<T>`/`Task<T>` return/param → tagged external edge) are correct tagged external facts, consistent with a non-generic external reference (`HttpClient`) — kept, not suppressed as redundant with the identifier-path type-argument edge. Reviewed `IsNamedTypeRefEligible`'s existing comment (predefined/array/nullable/tuple SYNTAX only) and confirmed it already scopes the primitive-noise exclusion correctly — it does not claim to exclude BCL types wholesale, so no correction was needed.

### Tests

- **`tests/HeritageExternalTaggingReviewFixesTests.cs`** (new, 5 fixtures) — one per finding, TDD RED witnessed first against the pre-fix code: `UnresolvableHeritageBase_...` (finding #1: `"unresolved-type-ref"` present, `"unresolved-reference"` absent), `MethodTypeParameter_...` + `GenericConstraint_TypeParameterBound_...` (finding #2: 2 and 1 spurious `unresolved-type-ref` Limitations respectively, pre-fix), `ExternalGenericReturnType_TargetName_HasNoGlobalToken` (finding #3: target `system-collections-generic-list-global-insourcetype`, pre-fix), `ExternalGenericReturnType_EdgeIsKept_NotSuppressed` (finding #3a control, passed pre-fix as expected — a regression guard). GREEN after the fix, 5/5.
- Full suite: **297/297 pass** (was 292, +5 from this row's fixtures, 0 changed elsewhere). `dotnet build src -c Debug` clean; `dotnet publish` (`npm run build`) clean.

### Pre-prod migration note

Drop-path Limitations from the 0.63.0 fix that silently vanished at ingest now surface under `kind: "unresolved-reference"`; type-parameter shapes that spuriously recorded a Limitation now record none — no migration path is provided (pre-prod convention): delete `.fathom/graph.db` and re-analyze.

### Scope note

This row's contract is the analyzer fix + its regression tests only, confined to `nodegraph-analyzer-dotnet`. `nodegraph-analyzer-conformance` fixtures referencing the old `"unresolved-type-ref"` kind (e.g. `J2-generic-constraint-unresolved`, which pins the OLD — now-corrected — type-parameter-as-unresolved behavior) and any `fathom-cli` coordination are OUT OF SCOPE and left for a separate pass.

## [0.63.0] — 2026-07-17

**Fathom rows 3.1.0.15 + 3.1.0.16 (unified) + 3.1.0.17 RC1 (crit 4) — C# external-base UNTAGGED emission hard-failed `fathom analyze` on real C# apps.** Heritage (`extends`/`implements`), `overrides`, and `references` (return-type/parameter-type/generic-constraint) decided emission on `allNames` — a name-EXISTENCE set — as if it were an element-key oracle. An edge to an EXTERNAL (BCL/NuGet/framework) base type then emitted with a bare `targetName`, no external tag: ingest synthesizes a bogus self-file `targetRef` that neither resolves nor is exempt from the strict edge gate, hard-failing `fathom analyze` (exit 1) on EnvisionWeb and MyPatientNow. Diagnosed as 3 stacked root causes, all in `src/Program.cs`: **RC1** an in-file attribute usage (`GetDeclarationName`'s `AttributeSyntax` arm) fed its bare name into `allNames`, so an external base sharing that short name slipped past the `!allNames.Contains(name) && targetFile == null` gate as if it were in-source; **RC2** (the majority shape, 33/42 measured) the `overrides` block had no gate and no external tag at all — an override of any resolved-external member (`Page.OnInit`, `Object.Equals`, `IDisposable.Dispose`, …) fell to a bare untagged emit; **RC3** the heritage/`references` sites keyed on the syntactic short name and never qualified nested types, so a base/interface nested in another class emitted as `inner` not `outer/inner`, and the `I`+uppercase interface heuristic mis-classified an `I`+digit interface (`I3D`) as a class base.

### Fixed

- **`src/Program.cs`, `ResolveTypeRef`** (new) — a symbol-based 3-way verdict (in-source / external / unresolved) for a type-position syntax reference, replacing the `allNames` gate. In-source: `INamedTypeSymbol` with a real declaration → qualified key (via `GetQualifiedRawName`, handling nested `outer/inner` + cross-file uniformly) + declaring file. External: a real symbol with no in-source declaration (BCL/NuGet) → its fully-qualified display name, no targetRef. Unresolved: `null`/`IErrorTypeSymbol` (`TypeKind.Error`) → the edge is DROPPED and a `Limitation` (`kind: "unresolved-type-ref"`) is recorded instead of a phantom bare-name edge.
- **Heritage** (`extends`/`implements`) — routed through `ResolveTypeRef`; external bases now emit `AddExternal`-shaped (`metadata.external` + `resolutionProvenance: "external-library"`), in-source bases emit the qualified `targetRef` (fixing RC3's nested-type mismatch), unresolved bases drop with a Limitation. The `I`+uppercase-second-char name heuristic is replaced by the symbol's actual `TypeKind == TypeKind.Interface`, fixing the `I`+digit mis-classification (RC3).
- **`overrides`** — when the overridden parent has NO declaring syntax reference at all (`parentDeclRef == null`, i.e. a genuinely external/BCL member), the edge now carries the external tag instead of falling to the old bare `else` emit. The same-file in-source bare-emit path (targetRef intentionally omitted per protocol) is unchanged.
- **`references` return-type / parameter-type / generic-constraint** (3.1.0.16, same escape) — routed through `ResolveTypeRef` via a shared `EmitTypeRefEdge` helper. External constraint/return/parameter types now emit tagged instead of silently dropping.
- **RC1 root cut (3.1.0.17)** — the `allNames` population pass now skips `AttributeSyntax` nodes entirely, closing the pollution at its source. `allNames`'s two remaining consumers (the in-file-call-resolution Limitation fallback and the event-handler `+=` fallback) are existence checks over METHOD/handler names — attributes are never called as methods, so the narrowing is safe for both and was verified not to change either's test results.
- **New guard, `IsNamedTypeRefEligible`** — predefined (`void`/`int`/`string`/`bool`/`object`/…), array, nullable, and tuple type SYNTAX are excluded from the `references` return-type/parameter-type symbol resolution. Confirmed empirically that every C# predefined-type keyword resolves to a genuine external `INamedTypeSymbol` (`System.Void`, `System.Int32`, …) with zero declaring syntax references — without this guard the fix would have manufactured a `references` edge to a compiler primitive on nearly every method in a C# corpus, a new noise class the pre-fix `allNames` gate never produced and that is out of this fix's scope (declared/framework base types, not language primitives).
- **`src/Program.Provenance.cs`** — new `ProvenanceHelpers.ClassifyExternalType`, mirroring `ClassifyExternalCall`/`ClassifyExternalProperty` for the type-reference edge families (always `external-library` — a type has no `dynamic`-indexer analogue).

### Tests

- **`tests/HeritageExternalTaggingTests.cs`** (new, 7 fixtures) — one per failure-shape category named in the design, plus an in-source control: attribute-polluted external base (RC1), external override (RC2, the majority shape), implicit external-interface implementation, nested same-file heritage (RC3), `I`+digit interface (RC3's heuristic mis-classification), cross-file nested heritage, and an in-source same-file/cross-file control guarding against over-tagging. **RED witnessed first**: `dotnet test --filter HeritageExternalTaggingTests` against the pre-fix analyzer DLL failed 6/7 for the intended reasons (bare untagged edges with `External=False`, a missing `implements` edge, an `extends`-not-`implements` mis-classification, and a nested-key mismatch — `"...outer2:inner2"` expected vs `"...inner2"` actual); the in-source control passed pre-fix as expected (a regression guard, not new behaviour). GREEN after the fix, 7/7.
- Full suite: **292/292 pass** (was 285, +7 from this row's fixtures, 0 changed elsewhere). `dotnet build src -c Debug` clean; `npm run build` (the `dotnet publish` wrapper) clean.
- Manually probed (not asserted in a test): a predefined/BCL-heavy method (`void`/`int`/`string`/`bool(int,string)`/`object`/`EventArgs` return types) emits exactly ONE `references` edge — the `EventArgs` one, correctly external-tagged — confirming the primitive-noise guard; a genuinely-unresolvable heritage base (`TotallyUndefinedBase`, no declaration anywhere) emits NO edge and records the new `unresolved-type-ref` Limitation instead of a phantom.

### Pre-prod migration note

External heritage/`overrides`/`references` edges that previously emitted BARE (untagged, danglING) or were silently DROPPED now carry `metadata.external` + `resolutionProvenance`, or drop with a loud `Limitation`, respectively — no migration path is provided (pre-prod convention): delete `.fathom/graph.db` and re-analyze.

### Scope note

This row's contract is the analyzer fix + its regression tests only. Re-arming the `fathom-cli` strict-edge-gate C#-heritage exclusions (the interim shipped 2026-07-17 alongside `nodegraph-analysis@3.73.0`/`fathom-cli@4.80.0`) and any `nodegraph-analyzer-conformance` fixture work are OUT OF SCOPE for this change — left for the orchestrator to sequence once this package is relinked and re-verified against a real C# corpus.
